...
class FBBot extends EventEmitter {
    constructor(opts) {
        ...
        opts = opts || {}
        if (!opts.token) {
            throw new Error('Missing page token. See FB documentation')
        }
    ...
    setPersistentMenu (payload, cb) {
        return thus.setField('persistent_menu', payload, cb)
    }
    ...
    removeDomainWhiteList (cb) {
        return this.deleteField('whilteListed_domains', cb)
    }
    removeGreeting (cb) {
    ...
    middleware() {
        return (req, res) =>{
            // we always write 200, otherwise facebook will keep retryling the request
            ...
            _getActionsObject (event) {
                return {
                    ...
                    markRead: this.sendSenderAction.bind(this.eventNames.sender.id,'mark_seen')
                }
            }
